...
// static int jsoneq2(const char *json,jsmntok_t *tok) {
//
// 	printf("%s",json + tok->start);
// 	return -1;
// }
...
int main() {
	...
	// int nameTokenCount =jsonNameList(JSON_STRING,&t[0],r,nameTokIndex);
	// char* name= "name";
	// int cc =pullTokindex(JSON_STRING,&t[0],r,nameTokIndex,3,name,nameTokenCount);
	// printf("%.*s",t[cc].end-t[cc].start, JSON_STRING + t[cc].start);

	PrintListOfSpecificProduct(JSON_STRING,&t[0], r);
	return EXIT_SUCCESS;
}
